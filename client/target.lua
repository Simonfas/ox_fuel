...
local function canUseOxFuel(vehicle)
	if not vehicle or vehicle == 0 then return false end
	if not DoesVehicleUseFuel(vehicle) then return false end

	local model = GetEntityModel(vehicle)

	if config.blacklistedVehicles and config.blacklistedVehicles[model] then
		return false
	end
	...
	exports.ox_target:addModel(config.pumpModels, {
		{
			...
			canInteract = function(entity)
				...
				return state.lastVehicle
					and canUseOxFuel(state.lastVehicle)
					and #(GetEntityCoords(state.lastVehicle) - GetEntityCoords(cache.ped)) <= 3
			end
		},
		{
			distance = 2,
	...
	exports.ox_target:addModel(config.pumpModels, {
		{
			...
			label = locale('start_fueling'),
			canInteract = function(entity)
				if state.isFueling or cache.vehicle or not canUseOxFuel(state.lastVehicle) then
					return false
				end
	...
	exports.ox_target:addGlobalVehicle({
		{
			...
			label = locale('start_fueling'),
			canInteract = function(entity)
				if state.isFueling or cache.vehicle or lib.progressActive() or not canUseOxFuel(entity) then
					return false
				end
